...
/* ---------------------------------------------------------------------
   ...
   Each section owns a ScrollTrigger that (a) shows/hides its 3D model
   group via onToggle and (b) scrubs a camera + rotation interpolation
   via onUpdate. Only the hero is pinned, per GSAP guidance on not
   over-pinning a scroll page.
   --------------------------------------------------------------------- */

const { phone, chip, cameraMacro, buttonMacro } = scene.models;
const groups = { phone, chip, cameraMacro, buttonMacro };
...
const sectionConfigs = [
  ...
  },
  {
    id: "controls",
    group: "buttonMacro",
    from: { camPos: [2.1, 0, 3.7], rotY: -0.4 },
    to: { camPos: [1.15, 0, 2.0], rotY: 0.6 },
  },
  {
    id: "colors",
    group: "phone",
...
if (swatchWrap) {
  IPHONE_COLORS.forEach((c) => {
    ...
    btn.style.background = `#${c.back.toString(16).padStart(6, "0")}`;
...
/* ---------------------------------------------------------------------
   Lightweight text reveal (kept separate from the GSAP camera rig so a
   layout-only concern can't fight the render-loop-driven 3D animation).
   --------------------------------------------------------------------- */
